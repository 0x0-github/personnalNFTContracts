...
import { NFTContractWithExtra } from "../types";
...
describe("ERC721ExtraData", async () => {
  const presaleDuration = 1200;
  const presalePrice = 1;
  const salePrice = 2;
  const maxSupply = 30;
  const maxMintTx = 10;
  const maxMintPresale = 5;
  const maxMintSale = 5;
  ...
  let nftContract: NFTContractWithExtra;
  ...
  beforeEach(async () => {
    const NFTContract = await ethers.getContractFactory("NFTContractWithExtra");
    const merkleRoot = whitelistTree.getHexRoot();

    presaleStart = (await ethers.provider.getBlock("latest")).timestamp;

    saleStart = presaleStart + presaleDuration;
    ...
    baseConfStruc = {
      ...
      salePrice: salePrice,
      maxMintTx: maxMintTx,
      maxMintPresale: maxMintPresale,
      maxMintSale: maxMintSale
    };

    nftContract = await NFTContract.deploy() as NFTContractWithExtra;

    await nftContract.deployed();

    await waitFor(nftContract.setUnrevealURI(UNREVEAL_URI));
    await waitFor(nftContract.setConfig(await nftContract.extraDataToUint256(baseConfStruc)));
    await waitFor(nftContract.setMerkleRoot(merkleRoot));
  });
